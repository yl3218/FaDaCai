...
def post_create(request):
    if request.method == "POST":
        ...
        new_class_time = request.POST.get('class_time')
        new_grade = request.POST.get('grade')
        new_comment = request.POST.get('comment')

        new_post = Post(title=new_title, teacher=new_teacher,
                        class_time=new_class_time, grade=new_grade, comment=new_comment)
...
def course(request):
    if requests.method == "POST":
        new_name = requests.POST.get('name')
        new_secore = Course(name=new_name)
...
def crawler(request):  # 爬蟲程式
    ...
    link = sp.find("table", {"class": "b-list"}
                   ).findAll("a", href=re.compile('C.php?'))
    for linkk in link:
...
def content(aa):
    ...
    authors = sp.find(
        'div', {"class": "c-post__header__author"}).findAll("a", {"class": "username"})
...
def sql():
    ...
    try:
        ...
        unit = gamee.objects.create(
            cAuthor=cAuthor, cContent=cContent, cTitle=cTitle, cLink=cLink)
        unit.save()  # 寫入資料庫
        print('成功儲存一筆資料')
        ...
        if register_form.is_valid():  # 驗證數據，提取表單內容
            ...
            if password1 != password2:  # 若兩次密碼不同
                ...
                same_name_user = models.User.objects.filter(
                    name=username)  # 比對資料庫是否有相同用戶名
                if same_name_user:
                    message = "該用戶名稱已存在!"
                    return render(request, 'register.html', locals())
                same_email_user = models.User.objects.filter(
                    email=email)  # 比對資料庫是否有相同信箱
                if same_email_user:
                    message = "信箱已被使用!"
                    return render(request, 'register.html', locals())
            # 若上面條件皆通過，則創建新的用戶
